...
$myLog = $PSScriptRoot + '/logs/' + $model_type.ToString() + '/'
...
foreach($model_type in $model_types)
{
    foreach($val in $values)
    {
        foreach ($input_file in $files)
        {
            ...
            Start-Process -FilePath "${PSScriptRoot}/x64/Release/AirplaneSeatAssignment.exe" -ArgumentList '-n', $val, '-m', $model_type, $input_file_name  -RedirectStandardOutput $output_file -RedirectStandardError $stdErrLog -wait
        }
    }
}